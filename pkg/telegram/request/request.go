...
func CheckIsCaptcha(valueUser int64) (bool, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "user/checkIsCaptcha")
	userCheckIsCaptcha := UserCheckIsCaptcha{
		ValueUser: valueUser,
	}
	userCheckIsCaptchaJson, err := json.Marshal(userCheckIsCaptcha)
	if err != nil {
		return false, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(userCheckIsCaptchaJson))
	if err != nil {
		return false, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var userCheckIsCaptchaResponse UserCheckIsCaptchaResponse
		err = json.Unmarshal([]byte(body), &userCheckIsCaptchaResponse)
		if err != nil {
			return false, err
		}
		if userCheckIsCaptchaResponse.Status == 200 && userCheckIsCaptchaResponse.Result {
			return true, nil
		} else {
			return false, nil
		}
...
func UpdateTradingAsset(teleId int64, chooseAsset string, choosePrice float64) (bool, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "user/updateTradingAsset")
	userUpdateTradingAsset := UserUpdateTradingAsset{
		TeleId:      teleId,
		ChooseAsset: chooseAsset,
		ChoosePrice: choosePrice,
	}
	userUpdateTradingAssetJson, err := json.Marshal(userUpdateTradingAsset)
	if err != nil {
		return false, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(userUpdateTradingAssetJson))
	if err != nil {
		return false, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var userUpdateTradingAssetResponse UserUpdateTradingAssetResponse
		err = json.Unmarshal([]byte(body), &userUpdateTradingAssetResponse)
		if err != nil {
			return false, err
		}
		if userUpdateTradingAssetResponse.Status == 200 {
			return true, nil
		}
	}
...
func UpdateTradingInvestPrice(teleId int64, investmentPrice float64) (bool, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "user/updateTradingInvestPrice")
	userUpdateTradingInvestPrice := UserUpdateTradingInvestPrice{
		TeleId:          teleId,
		InvestmentPrice: investmentPrice,
	}
	userUpdateTradingInvestPriceJson, err := json.Marshal(userUpdateTradingInvestPrice)
	if err != nil {
		return false, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(userUpdateTradingInvestPriceJson))
	if err != nil {
		return false, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var userUpdateTradingInvestPriceResponse UserUpdateTradingInvestPriceResponse
		err = json.Unmarshal([]byte(body), &userUpdateTradingInvestPriceResponse)
		if err != nil {
			return false, err
		}
		if userUpdateTradingInvestPriceResponse.Status == 200 {
			return true, nil
		}
	}
...
func UpdateTradingWaitTime(teleId int64, waitingTimeSec int) (bool, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "user/updateTradingWaitTime")
	userUpdateTradingWaitTime := UserUpdateTradingWaitTime{
		TeleId:         teleId,
		WaitingTimeSec: waitingTimeSec,
	}
	userUpdateTradingWaitTimeJson, err := json.Marshal(userUpdateTradingWaitTime)
	if err != nil {
		return false, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(userUpdateTradingWaitTimeJson))
	if err != nil {
		return false, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var userUpdateTradingWaitTimeResponse UserUpdateTradingWaitTimeResponse
		err = json.Unmarshal([]byte(body), &userUpdateTradingWaitTimeResponse)
		if err != nil {
			return false, err
		}
		if userUpdateTradingWaitTimeResponse.Status == 200 {
			return true, nil
		}
	}
...
func UpdateTradingMovePrice(teleId int64, movementPrice string) (bool, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "user/updateTradingMovePrice")
	userUpdateTradingMovePrice := UserUpdateTradingMovePrice{
		TeleId:        teleId,
		MovementPrice: movementPrice,
	}
	userUpdateTradingMovePriceJson, err := json.Marshal(userUpdateTradingMovePrice)
	if err != nil {
		return false, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(userUpdateTradingMovePriceJson))
	if err != nil {
		return false, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var userUpdateTradingMovePriceResponse UserUpdateTradingMovePriceResponse
		err = json.Unmarshal([]byte(body), &userUpdateTradingMovePriceResponse)
		if err != nil {
			return false, err
		}
		if userUpdateTradingMovePriceResponse.Status == 200 {
			return true, nil
		}
	}
...
func GetUserTradingParams(teleId int64) ([]TradingParams, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "user/getUserTradingParams")
	userGetUserTradingParams := UserGetUserTradingParams{
		TeleId: teleId,
	}
	userGetUserTradingParamsJson, err := json.Marshal(userGetUserTradingParams)
	if err != nil {
		return []TradingParams{}, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(userGetUserTradingParamsJson))
	if err != nil {
		return []TradingParams{}, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var userGetUserTradingParamsResponse UserGetUserTradingParamsResponse
		err = json.Unmarshal([]byte(body), &userGetUserTradingParamsResponse)
		if err != nil {
			return []TradingParams{}, err
		}
		if userGetUserTradingParamsResponse.Status == 200 && len(userGetUserTradingParamsResponse.Result) > 0 {
			return userGetUserTradingParamsResponse.Result, nil
		}
	}

	return []TradingParams{}, nil
}

func GetUserTotalEarn(teleId int64) ([]UserTotalEarn, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "user/getUserTotalEarn")
	userGetTotalEarn := UserGetTotalEarn{
		TeleId: teleId,
	}
	userGetTotalEarnJson, err := json.Marshal(userGetTotalEarn)
	if err != nil {
		return []UserTotalEarn{}, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(userGetTotalEarnJson))
	if err != nil {
		return []UserTotalEarn{}, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var userGetTotalEarnResponse UserGetTotalEarnResponse
		err = json.Unmarshal([]byte(body), &userGetTotalEarnResponse)
		if err != nil {
			return []UserTotalEarn{}, err
		}
		if userGetTotalEarnResponse.Status == 200 && len(userGetTotalEarnResponse.Result) > 0 {
			return userGetTotalEarnResponse.Result, nil
		}
	}

	return []UserTotalEarn{}, nil
}

func ChangeUserTotalEarn(teleId int64, earnMoney float64) (bool, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "user/changeUserTotalEarn")
	userChangeTotalEarn := UserChangeTotalEarn{
		TeleId:    teleId,
		EarnMoney: earnMoney,
	}
	userChangeTotalEarnJson, err := json.Marshal(userChangeTotalEarn)
	if err != nil {
		return false, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(userChangeTotalEarnJson))
	if err != nil {
		return false, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var userChangeTotalEarnResponse UserChangeTotalEarnResponse
		err = json.Unmarshal([]byte(body), &userChangeTotalEarnResponse)
		if err != nil {
			return false, err
		}
		if userChangeTotalEarnResponse.Status == 200 {
			return true, nil
		}
	}
...
func GetUserCurrentLuck(teleId int64) ([]UserCurrentLuck, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "user/getUserCurrentLuck")
	userGetCurrentLuck := UserGetCurrentLuck{
		TeleId: teleId,
	}
	userGetCurrentLuckJson, err := json.Marshal(userGetCurrentLuck)
	if err != nil {
		return []UserCurrentLuck{}, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(userGetCurrentLuckJson))
	if err != nil {
		return []UserCurrentLuck{}, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var userGetCurrentLuckResponse UserGetCurrentLuckResponse
		err = json.Unmarshal([]byte(body), &userGetCurrentLuckResponse)
		if err != nil {
			return []UserCurrentLuck{}, err
		}
		if userGetCurrentLuckResponse.Status == 200 && len(userGetCurrentLuckResponse.Result) > 0 {
			return userGetCurrentLuckResponse.Result, nil
		}
	}

	return []UserCurrentLuck{}, nil
}

func ChangeLuckUser(teleId int64, needLuck string) (bool, error) {
	checkNeedBase(os.Getenv("IS_TESTING"), "admin/adminChangeLuckUser")
	adminChangeLuckUser := AdminChangeLuckUser{
		TeleId:   teleId,
		NeedLuck: needLuck,
	}
	adminChangeLuckUserJson, err := json.Marshal(adminChangeLuckUser)
	if err != nil {
		return false, err
	}
	response, err := http.Post(baseUrl, contentType, bytes.NewBuffer(adminChangeLuckUserJson))
	if err != nil {
		return false, err
	}
	defer response.Body.Close()
	if response.StatusCode == 200 {
		...
		var adminChangeLuckUserResponse AdminChangeLuckUserResponse
		err = json.Unmarshal([]byte(body), &adminChangeLuckUserResponse)
		if err != nil {
			return false, err
		}
		if adminChangeLuckUserResponse.Status == 200 {
			return true, nil
		}
	}
