...
import (
	...
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/about"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/agreeTerms"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/changeLang"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/chooseCurrency"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/deposit"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/depositPayment"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/depositWrite"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/homeAfterReg"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/myNfts"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/myNftsAdminBuy"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/myNftsToken"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/myNftsTokenSell"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/nft"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/nftCollection"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/nftToken"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/nftTokenBuy"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/nickPayouts"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/profile"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/start"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/support"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/tradingAccount"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/tradingAccountMovePrice"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/tradingAccountPlayTraid"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/tradingAccountWaitTime"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/tradingAccountWriteSum"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/verification"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/withDraw"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/withDrawAdmApprove"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/withDrawAdmRefuse"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/withDrawPayment"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/withDrawWrite"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/addBalance"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/addMamMinim"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/addMammoth"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/blockUser"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/changeBalance"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/changeMamLuck"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/changeMamMinimal"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/changeMamPremium"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/changeMamVerification"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/findMammoth"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/mammothProfile"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/mammothsShow"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/messageMammoth"
	"github.com/rob-bender/invest-market-frontend/pkg/telegram/logics/workerPanel/myMammoths"
...
func (b *Bot) handleCommand(message *tgbotapi.Message) error {
	...
	case CommandStart:
		...
		if resCheckIsAdmin {
			...
			err := start.GetStart(b.Bot, b.SqliteDb, msg, message.From.ID, message.From.UserName, message.From.ID)
			if err != nil {
				return err
			}
		} else {
			if len(needParams) > 1 {
				...
				if resCheckIsAdmin {
					msg.ParseMode = "Markdown"
					err := start.GetStart(b.Bot, b.SqliteDb, msg, message.From.ID, message.From.UserName, i)
					if err != nil {
						return err
					}
				}
...
func (b *Bot) callbackQuery(callbackQuery tgbotapi.CallbackQuery) error {
	...
	case "NM_TA":
		...
		err = tradingAccount.TradingAccount(b.Bot, b.SqliteDb, msg, callbackQuery.Message.Chat.ID, callbackQuery.Message.Chat.UserName, resGetUserLang)
		if err != nil {
			return err
		}
	case "NM_TA_WS":
		...
		err = tradingAccountWriteSum.TradingAccountWriteSum(b.Bot, b.SqliteDb, msg, callbackQuery.Message.Chat.ID, callbackQuery.Message.Chat.UserName, resGetUserLang, needParams[1])
		if err != nil {
			return err
		}
	case "NM_TA_WT":
		...
		err = tradingAccountWaitTime.TradingAccountWaitTime(b.Bot, b.SqliteDb, msg, callbackQuery.Message.Chat.ID, callbackQuery.Message.Chat.UserName, resGetUserLang, needParams[1])
		if err != nil {
			return err
		}
	case "NM_TA_STRTD":
		...
		err = tradingAccountPlayTraid.TradingAccountPlayTraid(b.Bot, b.SqliteDb, msg, callbackQuery.Message.Chat.ID, callbackQuery.Message.Chat.UserName, resGetUserLang, i)
		if err != nil {
			return err
		}
	case "NM_WORKPANEL":
	...
	case "NM_WRKPNL_CCL":
		...
		err = changeMamLuck.ChangeMamLuck(b.Bot, msg, callbackQuery.Message.Chat.ID, callbackQuery.Message.Chat.UserName, resGetUserLang, i)
		if err != nil {
			return err
		}
	case "NM_WKPNL_U":
		...
		if len(params) == 2 {
			...
			fmt.Println("currentLuck -->", currentLuck)
			resChangeLuckUser, err := requestProject.ChangeLuckUser(i, currentLuck)
			if err != nil {
				return err
			}
			fmt.Println("resChangeLuckUser -->", resChangeLuckUser)
			if resChangeLuckUser {
				err = mammothProfile.MammothProfile(b.Bot, b.SqliteDb, msg, callbackQuery.Message.Chat.ID, callbackQuery.Message.Chat.UserName, resGetUserLang, i)
				if err != nil {
					return err
				}
...
func (b *Bot) handleMessage(message *tgbotapi.Message) error {
	...
	resCheckIsListenerWriteCaptcha, err := sqlite.CheckIsListenerWriteCaptcha(b.SqliteDb, message.Chat.ID)
	if err != nil {
		return err
	}
	if resCheckIsListenerWriteCaptcha {
		fmt.Println("message.Chat.ID -->", message.Chat.ID)
		...
		if err != nil {
			...
			resCheckIsCaptcha, err := requestProject.CheckIsCaptcha(i)
			if err != nil {
				return err
			}
			if resCheckIsCaptcha {
				err = sqlite.TurnOffListeners(b.SqliteDb, message.Chat.ID)
				if err != nil {
					return err
				}
				err := homeAfterReg.HomeAfterReg(b.Bot, b.SqliteDb, msg, message.Chat.ID, message.Chat.UserName, true, resGetUserLangTwo)
				if err != nil {
					return err
				}
			} else {
				...
				photo := tgbotapi.NewPhoto(message.Chat.ID, tgbotapi.FilePath(fmt.Sprintf("%s%d.jpg", needPath, resRandomRangeInt)))
	...
	resCheckIsListenerTradAcWriteSum, err := sqlite.CheckIsListenerTradAcWriteSum(b.SqliteDb, message.Chat.ID)
	if err != nil {
		return err
	}
	if resCheckIsListenerTradAcWriteSum {
		i, err := strconv.ParseFloat(message.Text, 64)
		if err != nil {
			if resGetUserLang == "ru" {
				msg.Text = "❌ *Введите число:*"
				msg.ReplyMarkup = keyboard.GenKeyboardInlineForTradingAcWriteSumBack("❌ Отмена")
			}
			if resGetUserLang == "en" {
				msg.Text = "❌ *Enter a number:*"
				msg.ReplyMarkup = keyboard.GenKeyboardInlineForTradingAcWriteSumBack("❌ Cancel")
			}
			_, err = b.Bot.Send(msg)
			if err != nil {
				return err
			...
			if len(resGetUserBalance) > 0 {
				...
				if len(resGetUserMinPrice) > 0 {
					if resGetUserMinPrice[0].MinimPrice > i {
						if resGetUserLang == "ru" {
							msg.Text = "❌ *Сумма инвестиции должна быть больше или равна минимальной!*\n\nВведите сумму:"
							msg.ReplyMarkup = keyboard.GenKeyboardInlineForTradingAcWriteSumBack("❌ Отмена")
						}
						if resGetUserLang == "en" {
							msg.Text = "❌ *You do not have enough funds on your account.*"
							msg.ReplyMarkup = keyboard.GenKeyboardInlineForTradingAcWriteSumBack("❌ Cancel")
						}
						_, err = b.Bot.Send(msg)
						if err != nil {
					...
					if resGetUserMinPrice[0].MinimPrice <= i {
						if resGetUserBalance[0].Balance < i {
							if resGetUserLang == "ru" {
								msg.Text = "❌ *На вашем счету недостаточно средств.*"
								msg.ReplyMarkup = keyboard.GenKeyboardInlineForTradingAcWriteSumBack("❌ Отмена")
							}
							if resGetUserLang == "en" {
								msg.Text = "❌ *You do not have enough funds on your account.*"
							...
							resUpdateTradingInvestPrice, err := requestProject.UpdateTradingInvestPrice(message.Chat.ID, i)
							if err != nil {
								return err
							}
							if resUpdateTradingInvestPrice {
								err = sqlite.TurnOffListeners(b.SqliteDb, message.Chat.ID)
								if err != nil {
									return err
								}
								err := tradingAccountMovePrice.TradingAccountMovePrice(b.Bot, b.SqliteDb, msg, message.Chat.ID, message.Chat.UserName, resGetUserLang)
								if err != nil {
									return err
								}
							}
	...
	resCheckIsListenerTradeGameStart, err := sqlite.CheckIsListenerTradeGameStart(b.SqliteDb, message.Chat.ID)
	if err != nil {
		return err
	}
	...
	case "NFT 🎆":
		...
		if !resCheckIsListenerTradAcWriteSum {
			if !resCheckIsListenerTradeGameStart {
				if !resCheckIsBlockUser {
				...
				if resGetUserLang == "ru" {
					msg.Text = "❌ Дождитесь окончания инвестиции перед тем, как делать что-либо еще!"
				}
				if resGetUserLang == "en" {
					msg.Text = "❌ Wait for the investment to end until you can do anything else!"
				}
				_, err := b.Bot.Send(msg)
				if err != nil {
					return err
	...
	case "📈 My trading account":
		...
		if !resCheckIsListenerTradAcWriteSum {
			if !resCheckIsListenerTradeGameStart {
				if !resCheckIsBlockUser {
					err := tradingAccount.TradingAccount(b.Bot, b.SqliteDb, msg, message.Chat.ID, message.Chat.UserName, resGetUserLang)
					if err != nil {
						return err
					}
				} else {
