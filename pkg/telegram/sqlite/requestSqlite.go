...
func TurnOnListenerWatchingWriteCaptcha(sqliteDb *sql.DB, teleId int64) error {
	_, err := sqliteDb.Exec("UPDATE bot_params SET watching_captcha_write = $1 WHERE tele_id = $2", 1, teleId)
	if err != nil {
		return err
	}
	return nil
...
func CheckIsListenerWriteCaptcha(sqliteDb *sql.DB, teleId int64) (bool, error) {
	type BotParams struct {
		WatchingWriteCaptcha bool `json:"watching_write_price"`
	}
	rows, err := sqliteDb.Query("SELECT watching_captcha_write FROM bot_params WHERE tele_id = $1", teleId)
	if err != nil {
		return false, err
	}
	botParams := []BotParams{}
	for rows.Next() {
		bp := BotParams{}
		err := rows.Scan(&bp.WatchingWriteCaptcha)
		if err != nil {
			continue
		}
		botParams = append(botParams, bp)
	...
	return botParams[0].WatchingWriteCaptcha, nil
}

func TurnOnListenerWatchingTradAcWriteSum(sqliteDb *sql.DB, teleId int64) error {
	_, err := sqliteDb.Exec("UPDATE bot_params SET watching_tradac_write_sum = $1 WHERE tele_id = $2", 1, teleId)
	if err != nil {
		return err
	}
	return nil
...
func CheckIsListenerTradAcWriteSum(sqliteDb *sql.DB, teleId int64) (bool, error) {
	type BotParams struct {
		WatchingTradAcWriteSum bool `json:"watching_tradac_write_sum"`
	}
	rows, err := sqliteDb.Query("SELECT watching_tradac_write_sum FROM bot_params WHERE tele_id = $1", teleId)
	if err != nil {
		return false, err
	}
	botParams := []BotParams{}
	for rows.Next() {
		bp := BotParams{}
		err := rows.Scan(&bp.WatchingTradAcWriteSum)
		if err != nil {
			continue
		}
		botParams = append(botParams, bp)
	...
	return botParams[0].WatchingTradAcWriteSum, nil
}

func TurnOnListenerWatchingTradeGameStart(sqliteDb *sql.DB, teleId int64) error {
	_, err := sqliteDb.Exec("UPDATE bot_params SET watching_tradegame_start = $1 WHERE tele_id = $2", 1, teleId)
	if err != nil {
		return err
	}
	return nil
...
func CheckIsListenerTradeGameStart(sqliteDb *sql.DB, teleId int64) (bool, error) {
	type BotParams struct {
		WatchingTradeGameStart bool `json:"watching_tradegame_start"`
	}
	rows, err := sqliteDb.Query("SELECT watching_tradegame_start FROM bot_params WHERE tele_id = $1", teleId)
	if err != nil {
		return false, err
	}
	botParams := []BotParams{}
	for rows.Next() {
		bp := BotParams{}
		err := rows.Scan(&bp.WatchingTradeGameStart)
		if err != nil {
			continue
		}
		botParams = append(botParams, bp)
	...
	return botParams[0].WatchingTradeGameStart, nil
}
